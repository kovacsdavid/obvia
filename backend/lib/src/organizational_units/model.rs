...
/// Represents an organizational unit with database configuration and metadata.
///
/// This struct is used to store information about an organizational unit,
...
/// - `created_at` (chrono::DateTime<chrono::Local>): Timestamp when the organizational unit was created.
/// - `updated_at` (chrono::DateTime<chrono::Local>): Timestamp when the organizational unit was last updated.
/// - `deleted_at` (Option<chrono::DateTime<chrono::Local>>): Optional timestamp indicating when the
///   organizational unit was soft deleted (if applicable).
...
/// A struct representing the relationship between a user and an organizational unit in the application.
///
/// This structure is typically used to capture and manage data related to user roles
/// within specific organizational units. It also maintains metadata about who invited
/// the user to the organizational unit, as well as timestamps for creation, updates,
...
/// - `organizational_unit_id` (`Uuid`): The unique identifier of the organizational unit
///   which the user is a part of.
/// - `role` (`String`): The role assigned to the user within the organizational unit
///   (e.g., "owner", "member", etc.).
/// - `invited_by` (`Option<Uuid>`): The unique identifier of the user who invited this
///   user to the organizational unit, if applicable.
