...
pub async fn print<M: ServicesModule>(
    ...
    let service = Service::new(Some(&claims), services_module.clone());
    let service_resolved_print = ServicesResolvedPrint::new(
        map_handler_err(
            service.get_resolved(payload.uuid).await,
            services_module.clone(),
        )
...
mod tests {
    ...
    async fn test_print_success() {
        let active_tenant_id = Uuid::new_v4();
        let service_id = "4f321721-37c6-4e91-8e42-6281c36937bc".parse().unwrap();
        ...
        let pdf_gen_payload_expected = vec![ServicesResolvedPrint::new(
            service_resolved,
        ...
        let snapshot_path = Path::new("testing/pdf/snapshots/services_test.pdf");
        ...
        let body_bytes = axum::body::to_bytes(response.into_body(), usize::MAX)
            ...
            .unwrap()
            .to_vec();
        let test_result_path = Path::new("testing/pdf/test_results/services_test.pdf");
