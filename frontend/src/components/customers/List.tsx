...
import {type CustomerResolvedList, isPaginatedCustomerResolvedListResponse} from "@/components/customers/interface.ts";
      ...
      if (response.meta.requestStatus === "fulfilled") {
        ...
        try {
          ...
          switch (payload.status) {
            case 200: {
              if (isPaginatedCustomerResolvedListResponse(responseData)) {
        ...
        <TableBody>
          {data.map((item) => (
            <TableRow key={item.id}>
              ...
              <TableCell>{item.phone_number ? item.phone_number : 'N/A'}</TableCell>
