...
describe('useNumberInput', () => {
  ...
  it('should initialize with provided initial value', () => {
    const {result} = renderHook(() => useNumberInput({initialValue: '1234.56'}));
  ...
  it('should handle numbers without thousand separator', () => {
    const {result} = renderHook(() => useNumberInput({showThousandSeparator: false}));
  ...
  it('should handle cursor position with thousand separators', () => {
    const mockInput = document.createElement('input');
    const {result} = renderHook(() => useNumberInput({showThousandSeparator: true}));
