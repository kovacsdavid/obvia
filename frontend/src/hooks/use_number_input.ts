...
export function useNumberInput(options: UseNumberInputOptions = {}) {
  ...
  const [displayValue, setDisplayValue] = useState(() =>
    formatNumber(initialValue, {showThousandSeparator, decimalPlaces, allowEmpty})
  );
  ...
  const handleInputChange = useCallback((inputValue: string) => {
    ...
    if (decimalPlaces !== undefined || inputValue.includes(',') || inputValue.includes('.')) {
      // When dealing with decimals, only format if the input looks complete
      // or if we're not in the middle of typing a decimal
      const hasIncompleteDecimal = inputValue.endsWith(',') || inputValue.endsWith('.');
  ...
  const handleInputChangeWithCursor = useCallback((
    ...
    if (inputElement && showThousandSeparator) {
      setTimeout(() => {
        ...
        const oldSpacesBefore = (oldValue.substring(0, cursorPosition).match(/\s/g) || []).length;
        const newSpacesBefore = (newValue.substring(0, cursorPosition).match(/\s/g) || []).length;
  ...
  }, [displayValue, handleInputChange, showThousandSeparator, decimalPlaces, allowEmpty]);
